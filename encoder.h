        ...
        if( raw_packet.length() > pos_encoder + 23 )
        {
         //   std::cout << "loop check(start)"<<std::endl;
            ...
            else
            {
                ...
                if(abs(diff.left_top) < ERR_GAP*10 )      prev.left_top = curr.left_top;
                else                                      curr.left_top = prev.left_top;

                if(abs(diff.right_top) < ERR_GAP*10 )     prev.right_top = curr.right_top;
                else                                      curr.right_top = prev.right_top;

                if(abs(diff.left_bottom) < ERR_GAP*10 )   prev.left_bottom = curr.left_bottom;
                else                                      curr.left_bottom = prev.left_bottom;

                if(abs(diff.right_bottom) < ERR_GAP*10 )  prev.right_bottom = curr.right_bottom;
                else                                      curr.right_bottom = prev.right_bottom;
